...
section .text
  ...
  load_level:
    ...
    read [lvl_file_h], level_buf, LVL_BUF_LEN   ; read file contents into buffer
  ...
  .check_char:
    xor rax, rax                                ; rax = 0
    mov byte al, [level_buf + r10]              ; al = level_buf[r10]
    cmp al, 'z'                                 ; if al == 'z'
  ...
  .check_if_wall:
    xor rdx, rdx                                ; rdx = 0
    mov byte dl, [level_buf + rax]              ; dl = level_buf[rax]
    cmp dl, 'a'                                 ; if dl < 'a'
  ...
  lvl_consume:
    ...
    add rsp, 16                                 ; pop args
    xor rdx, rdx                                ; rdx = 0
    mov rdx, [level_buf + rax]                  ; rdx = level_buf[rax]
    mov byte [level_buf + rax], ' '             ; level_buf[rax] = ' '
    cmp dl, '.'                                 ; if dl == '.'
    je .dot                                     ;   goto .dot
    cmp dl, ','                                 ; else if dl == ','
    je .power                                   ;  goto .power
